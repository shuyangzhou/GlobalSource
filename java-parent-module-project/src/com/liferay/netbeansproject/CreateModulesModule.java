...
public class CreateModulesModule {
		...
		if (!importShared.isEmpty()) {
			...
			for (String module : importSharedList) {
				if (!importSharedSet.contains(module)) {
					...
					String test = ModuleBuildParser.modulePathFinder(
						module, projectInfo.getPortalDir() + "/modules");

					_appendImportSharedList(importSharedSet, projectInfo, test);
				}
			}
		}
	}
	...
	private static void _appendProperties(
		...
		try (
			...
			singleModuleList.addAll(
				Arrays.asList(singleModuleProperty.split(",")));
			...
			for (String module : importShared) {
				if (!module.equals("")) {
					...
					String startPath = projectInfo.getPortalDir() + "/modules";
					File importModule = CommonTasks.modulePathFinder(
						startPath, module);

					File[] importModuleFiles = importModule.listFiles();

					for (File importModuleFile : importModuleFiles) {
	...
	private static void _createData(
		...
		for (File moduleFile : moduleFiles) {
			...
			if (new File(
					projectInfo.getFullPath(
						) + "/" + moduleFileName + "/test/unit").exists()) {
			...
			if (new File(
					projectInfo.getFullPath(
						) + "/" + moduleFileName + "/test/integration").exists(
							)) {
